...
class Search_model extends CI_Model {
    ...
    public function get($get, $page = 1) {
        ...
        if ($keyword != '') {
            $temp = array();
            $sfield = array('title', 'keywords', 'shx', 'sdlx', 'wslxx', 'gstxx', 'jx', 'ssfww', 'qhlx');
            foreach ($sfield as $t) {
    ...
    private function _where($table, $name, $value, $field) {
        if (strpos($value, '%') === 0
            ...
            if ($data) {
                if ($data['child']) {
                    return '$name'.$table.'`.`'.$name.'` IN ('.$data['childids'].')';
                } else {
            ...
            if(is_array($value))
            {
                ...
                foreach ($value as $key => $val)
                {
                    ...
                    if($i ==1 )
                    {
                        $str =" find_in_set('".$val."',REPLACE(REPLACE(REPLACE(`".$table."`.`".$name."`,'[',''),']',''),'\"',''))";
                    }else{
                        $str .=" AND find_in_set('".$val."',REPLACE(REPLACE(REPLACE(`".$table."`.`".$name."`,'[',''),']',''),'\"',''))";
                    }
            ...
            return '`' . $table . '`.`' . $name . '` in ( ' . stripslashes($this->db->escape_str($v)) . ')';
